import React from 'react';
import './page.css';
import SVG from 'react-inlinesvg';
import { data, chainIcons } from './constant';
import logo from '../../Images/inoxxai/logo.png';

const Page = () => {
  return (
    <>
      <div className='container'>
        <div className='line_block_1'>
          <div className='line_circle_block_1'>
            <div className='line_inner_circle_block_1'></div>
          </div>
        </div>
        <div className='centerText'>
          Build your Dapps on a{' '}
          <span className='textHighlight'>Fast and a Secure Blockchain</span>{' '}
          Platform
        </div>
        <div className='belowText'>
          Bitindi is a fast, high-throughput open-source smart contract platform
          for digital assets and Dapps.
        </div>
        <div className='arrowDiv'>
          <div className='arrowButton'>
            <div className='arrowText'>Read the whitepaper</div>
            <div className='arrowIcon'>
              <svg
                xmlns='http://www.w3.org/2000/svg'
                width='24'
                height='24'
                viewBox='0 0 24 24'
                fill='none'
              >
                <path
                  fill-rule='evenodd'
                  clip-rule='evenodd'
                  d='M3.75 12C3.75 11.5858 4.08579 11.25 4.5 11.25L17.6893 11.25L12.2197 5.78033C11.9268 5.48744 11.9268 5.01256 12.2197 4.71967C12.5126 4.42678 12.9874 4.42678 13.2803 4.71967L20.0303 11.4697C20.3232 11.7626 20.3232 12.2374 20.0303 12.5303L13.2803 19.2803C12.9874 19.5732 12.5126 19.5732 12.2197 19.2803C11.9268 18.9874 11.9268 18.5126 12.2197 18.2197L17.6893 12.75L4.5 12.75C4.08579 12.75 3.75 12.4142 3.75 12Z'
                  fill='#0F172A'
                />
              </svg>
            </div>
          </div>
          <div className='line'></div>
        </div>
        <div className='compareFrame'>
          <div className='ellipse_corner'>
            <div className='bar2'>
              <div className='bar1'></div>
            </div>
          </div>
          <div className='headingBlock'>Top Tier Platforms</div>
          <div className='compareView'>
            <div className='featureName'>
              <div className='featureHeading'>Features</div>
              <div className='featureList'>
                {Object.keys(data).map((el) => {
                  return <div className='listItems'>{el}</div>;
                })}
              </div>
            </div>
            <div className='products'>
              {Object.keys(data[Object.keys(data)[0]]).map((el) => {
                return (
                  <div className='productCol'>
                    {!(el === 'Cicca') ? (
                      <div className='productLogo'>
                        <SVG src={chainIcons[el]} title={el} />
                      </div>
                    ) : (
                      <div className='productImage'>
                        <img height={50} src={logo} alt='logo' />
                      </div>
                    )}
                    <div className='productInfo'>
                      {Object.keys(data).map((key) => {
                        return <div className='details'>{data[key][el]}</div>;
                      })}
                    </div>
                  </div>
                );
              })}
            </div>
          </div>
        </div>
      </div>
    </>
  );
};

export default Page;
